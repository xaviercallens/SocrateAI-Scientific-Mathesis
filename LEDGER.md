...
| `MX-B-0004` | Stream 0's own Lean tree contains **zero** custom `axiom` declarations. | `tests/tier_b_axiom_hygiene.py` (**2 negative controls**) | — |
...
| `MX-C-0003` | **Axiom survey, 2026-08-13.** Stream 1's `lean_src/`: **0** axioms. Stream 5's `DualScale/`: **34** across 14 files, against that stream's own CI Rule R1. Three load-bearing overlaps: `aubin_lions_compactness` — axiom there, hypothesis parameter in Stream 1; `dyadic_cascade_conservation` — axiom there, **proved** Tier A in Stream 1; `enstrophy` — axiomatized there, defined in Stream 1. Separately: the `Reff` duplicate `SPEC-STREAM0` §1 expects **does not exist**; Stream 5's nearest analogue instead fixes `alpha_prime := 1` and axiomatizes T-duality. | `docs/designs/RECONCILIATION.md` §3–§4 | — |
